...
import { ArrowUpRight, Briefcase, CheckSquare, DollarSign, FolderKanban, Zap } from "lucide-react";
  ...
  return (
    <main className="space-y-8">
      <header className="flex flex-col gap-6 rounded-[28px] border border-[#cfe1d8] bg-gradient-to-br from-white/70 to-[#f1faf7]/70 p-6 shadow-[0_22px_60px_rgba(15,23,42,0.04)] sm:p-8 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p className="text-sm font-medium uppercase tracking-[0.18em] text-[#0e5d53]">Overview</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-[-0.06em] text-slate-900 sm:text-5xl">
            Good morning, {firstName}
          </h1>
          <p className="mt-3 max-w-xl text-sm text-slate-600 sm:text-base">
            Here’s a quick view of your client work, delivery health, and what still needs attention.
          </p>
        </div>

        <div className="flex items-center gap-3">
          <Button variant="secondary" className="border border-[#cfe1d8] bg-[#f1faf7] text-[#0e5d53] hover:text-slate-900">
            View reports
          </Button>
          <Button asChild>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {summary.map((item, index) => {
          ...
          return (
            <div key={item.label} className="rounded-[24px] border border-[#cfe1d8] bg-gradient-to-br from-white/70 to-[#f1faf7]/65 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.03)]">
      ...
      <section className="grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
        <div className="rounded-[28px] border border-[#cfe1d8] bg-gradient-to-br from-white/70 to-[#f1faf7]/70 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.03)] sm:p-6">
          <h2 className="text-xl font-semibold text-slate-900">Recent clients</h2>
            <Link href="/dashboard/clients" className="inline-flex items-center gap-1 text-sm font-medium text-[#0e5d53]">
          ...
          {clientError ? (
            ...
            <div className="rounded-2xl border border-dashed border-[#cfe1d8] bg-[#e5f3f0] p-8 text-center">
              ...
              <p className="mt-2 text-sm text-slate-600">Add your first client to start building your pipeline.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {clients?.map((client) => (
                <div key={client.id} className="flex items-center justify-between gap-4 rounded-2xl border border-[#cfe1d8] bg-[#e5f3f0] px-3 py-3">
                  <div className="min-w-0">
                    <p className="truncate font-medium text-slate-900">{client.name}</p>
                    <p className="truncate text-sm text-[#0e5d53]">{client.company || "Independent"}</p>
                  </div>
                  <div className="text-right text-sm text-[#0e5d53]">
                    <p>{client.email || "No email"}</p>
                    <p className="mt-1">{new Date(client.created_at ?? Date.now()).toLocaleDateString()}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="rounded-[28px] border border-[#cfe1d8] bg-gradient-to-br from-white/70 to-[#f1faf7]/70 p-5 shadow-[0_18px_40px_rgba(15,23,42,0.03)] sm:p-6">
          ...
          <div className="mt-5 space-y-3">
            {[
              ...
              >
                ...
                <Badge variant={item.tone as "default" | "info" | "warning"}>{item.tone === "default" ? "New" : item.tone === "info" ? "Now" : "Draft"}</Badge>
              </button>
            ))}
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-[#bfddd2] bg-[#f1faf7] p-4">
            ...
            <p className="mt-2 text-sm text-slate-600">Your team is tracking steadily. Two tasks are due this week.</p>
          </div>
        </div>
      </section>
    </main>
