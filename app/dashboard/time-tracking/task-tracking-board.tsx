    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-[#cfe1d8] bg-white/80 p-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="grid grid-cols-2 gap-2 sm:flex">
          <select value={assigneeFilter} onChange={(event) => setAssigneeFilter(event.target.value)} className="min-w-0 rounded-full border border-[#cfe1d8] bg-white px-3 py-1.5 text-xs text-slate-700"><option value="all">Everyone</option>{assignees.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select>
...
function TaskCard({ task, assigneeName, projectName, onStatusChange }: { task: Task; assigneeName: string; projectName: string; onStatusChange: (id: string, status: string) => void }) {
  ...
  return <article className="rounded-2xl border border-[#cfe1d8] bg-[#f7faf8] p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h3 className="truncate font-semibold text-slate-900">{task.title}</h3><p className="mt-1 truncate text-xs text-slate-500">{projectName} · Assigned to {assigneeName}</p></div><select aria-label={`Status for ${task.title}`} value={task.status} onChange={(event) => onStatusChange(task.id, event.target.value)} className="w-full rounded-full border border-[#cfe1d8] bg-white px-2 py-1.5 text-xs text-slate-700 sm:w-auto">{TASK_STATUSES.map((status) => <option key={status} value={status}>{label(status)}</option>)}</select></div>{task.description ? <p className="mt-3 line-clamp-3 text-sm text-slate-600">{task.description}</p> : null}<div className="mt-4 flex items-center justify-between text-xs"><span className={isOverdue ? "font-medium text-rose-700" : "text-slate-500"}>{isOverdue ? "Overdue · " : "Due "}{task.due_date}</span>{task.status === "completed" ? <span className="font-medium text-emerald-700">Completed</span> : null}</div></article>;
}
